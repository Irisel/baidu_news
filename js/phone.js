...
define(function(){
    var phone_init = function(){
        ...
        $("#phone_champion").click(function(){
            var _this = this;
            excahnge_zones(_this);
            var video_position = $('#player_second').offset().top;
		    $.scrollTo(video_position - 50, 500);
	    });

        $("#phone_second").click(function(){
            var _this = this;
            excahnge_zones(_this);
            var video_position = $('#player_champion').offset().top;
		    $.scrollTo(video_position - 50, 500);
	    });
        $("#phone_third").click(function(){
            var _this = this;
            excahnge_zones(_this);
...
//	    });
	    ...
	    $("#phone_players").click(function(){
            var _this = this;
            excahnge_zones(_this);
            var main5 = $('.phone-main5').offset().top - 28;
            $.scrollTo(main5, 500);
	    });
	    ...
	    $("#phone_partners").click(function(){
            ...
            $.scrollTo(phone_footer - 50, 500);
	    });

        $("#phone_location").click(function(){
            var _this = this;
            excahnge_zones(_this);
            var phone_location = $('#ml-phone').offset().top;
            $.scrollTo(phone_location - 50, 500);
	    });
...
//               title: '口碑街',
//               size: 2,
//               par: '口碑街是由达人在社交网络中传播口碑的推广平台和新潮生活指南。口碑街是广告界的分享经济平台。商家在口碑街平台上投放offer后，普通用户可以领取并在自己的社交网络中分享、扩散，并按效果领取报酬。以此，商家就可以直达目标用户，以去广告化、去渠道化、碎片化的模式，发动大众来做广告。口碑街在追踪链接的防作弊、防刷点以及商家与用户的社交价值匹配方面有深度的技术支撑，并将进一步研究发展。通过严格精选广告商家，口碑街向用户展现的推广都是时下新潮、有趣的产品，用户进入网站，就好像掀开了未来生活的一角，用户可以选择自己喜爱的产品分享给朋友并获得奖励。这个奖励将被用于继续在口碑街站内换购产品。这样，用户在发现和分享的过程中，营造了口碑，传播了产品，获得了奖励。口碑街正在聚集一批高质量的消费意见领袖，为他们提供发现、分享和真正为自己代言的乐趣。'
//             },
             undefined,
             {
                name: '严伯钧',
...
//                size: 2,
        ...
        function html_pdg(info){
            ...
            var luyan = info.luyan?('<div class="luyan-div"><a class="luyan" href="' + info.luyan +'"></a></div>'):'<div class="luyan-div"></div>';
             return   '<div class="player"'+ style +'>' + img +'<span class="player-name">'+ info.name +'</span><div class="player-title"><span>'+
                info.title + '</span></div>' + luyan +'<p class="player-p">'+ info.par +'</p></div>';
        }
        ...
        function inlit_players2(){
              var list = $('#phone-player2').find('td');
              for(var i=0;i<players2.length;i++){
                  $(list[i]).html(html_pdg(players2[i]));
              }
        }

        function inlit_players3(){
              var list = $('#player16').find('td');
              for(var i=0;i<players16.length;i++){
                  $(list[i]).html(html_pdg2(players16[i]));
              }
        }
        ...
        for(var i=0;i<lists;i++){
            ...
            for(var j=i*4 ;j<i*4 + 4;j++){
                ...
                if(players[j])template+=html_pdg(players[j]);
            }
            template+='</div>';
            ...
            $('.point-pages').css('width', 16 * lists);
        }
...
//        $('.m3-bg5').css({'top': (main3.offset().top + main3.height()) -215 + 'px'});
        var lazy_imgs = $('img.lazy_jq');
        $.each(lazy_imgs, function(key, img){
